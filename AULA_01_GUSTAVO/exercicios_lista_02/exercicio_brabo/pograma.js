...
/*
   ...
   async function sincronizarDados() {
    ...
    const resultado = meuBanco.exec("SELECT id, apelido, especie FROM planta WHERE sincronizado = 0");
...
async function recuperarDadosDaNuvem() {
    ...
    if(data && data.length > 0){
        ...
        for(let i = 0; i < data.length; i++){
            ...
            meuBanco.run(
                "INSERT INTO planta (apelido, especie, sincronizado) VALUES (?,?,1)",
...
function salvarEstadoBanco(db){
    ...
    localStorage.setItem("plantas_banco", JSON.stringify(array));
}
...
btn.addEventListener('click', function(event) {
    ...
    const apelido = document.getElementById("textoApelido").value;
    const especie = document.getElementById("textoEspecie").value;

    if(meuBanco && apelido && especie){

        meuBanco.run(
            "INSERT INTO planta (apelido, especie) VALUES (?,?)",
            [apelido, especie]
        );
...
btnExcluir.addEventListener('click', async function (event) {
    ...
    await supabaseClient.from('planta').delete().eq('id', id);

    
    meuBanco.run("DELETE FROM planta WHERE id = ?", [id]);
    salvarEstadoBanco(meuBanco);
...
btnAlterar.addEventListener('click', function (event) {
    ...
    meuBanco.run(
        "UPDATE planta SET apelido = ?, sincronizado = 0 WHERE id = ?",
        [apelido, id]
