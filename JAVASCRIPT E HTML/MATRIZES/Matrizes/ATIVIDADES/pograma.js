...
get matriza(){
  ...
  gerarMatrizValoresAleatorios(){
    for (let linha = 0; linha < this.#qLinhas; linha++) {
      for (let coluna = 0; coluna < this.#qColunas; coluna++) {
        this.#matriz[linha][coluna] = Math.floor(Math.random() * 90 + 10); // preenche e incrementa
      }
    }
 ...
 for (let linha = 0; linha < this.#qLinhas; linha++) {
  ...
  gerarMatrizSubtracao(dadosMatriz1, dadosMatriz2){
...
matrizC.gerarMatrizSoma(matrizA.matriza,matrizB.matriza);
console.log(matrizC.mostrarMatriz());
console.log("SUBTRAÇÃO DAS MATRIZES: ");
